...
type Block struct {
	...
	**/
	...
	// 使用部分唯一索引：只对 page 类型、未删除且非 NULL 的 slug 生效
	Slug *string `gorm:"type:varchar(255);uniqueIndex:idx_blocks_slug_unique,where:deleted_at IS NULL AND slug IS NOT NULL AND type = 'page'" json:"slug,omitempty"`

	// 发布时间控制：NULL 表示草稿，非 NULL 表示已发布
	PublishedAt *time.Time `gorm:"type:timestamptz;index" json:"published_at,omitempty"`

	// 分类归属（仅 Page 类型使用，NULL 表示未分类）
