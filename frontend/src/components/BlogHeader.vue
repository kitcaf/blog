...
<template>
  <header class="flex items-center justify-between mb-12">
    <router-link to="/" class="text-2xl text-[var(--color-fg-deeper)] transition-colors duration-200">
      kitcaf
    </router-link>
    ...
    <nav class="flex items-center gap-8 text-sm font-medium" style="view-transition-name: nav-links">
      ...
      <div class="flex items-center gap-4 text-[var(--color-fg-light)] ml-2">
        <a href="https://github.com/kitcaf" class="hover:text-[var(--color-fg-deeper)] transition-colors"><svg
        ...
        <button @click="toggleTheme"
          class="hover:text-[var(--color-fg-deeper)] transition-colors duration-200 flex items-center cursor-pointer"
