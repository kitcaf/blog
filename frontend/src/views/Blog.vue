      ...
      <Transition name="list-switch" mode="out-in">
        <div :key="activeCategory" class="space-y-1">
          <article v-for="post in filteredPosts" :key="post.slug" @click="goToDetail(post.slug)"
            ...
            <div class="w-40 flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity duration-500">
          ...
          <div v-if="filteredPosts.length === 0" class="py-20 text-center text-[var(--color-fg-light)] font-light">
