...
- 使用 `slug` 作为文章详情页路由标识，例如 `/blog/agent-future`，不再使用 `/blog/1` 这种纯 id 路由。
...
- Notion 未来生成的 slug 需要稳定，不能每次标题改动都导致旧链接失效。
...
- 已抽查 `dist/blog/agent-future/index.html`，源码中包含文章标题、摘要、正文、页面 title、meta description、canonical 和 OG 信息。

## Notion phase input mapping

- Notion 标题字段映射到 `title`，构建脚本负责生成或读取稳定 `slug`。
